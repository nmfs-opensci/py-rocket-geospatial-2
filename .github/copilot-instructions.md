...
- **`conda-env/env-*.yml`**: Five separate conda environment files installed sequentially to avoid solver issues:
...
### Auto-Generated Package Pins (in reproducibility/)

- **`reproducibility/packages-python-pinned.yaml`**: Auto-generated list of Python packages from env-*.yml files with exact versions
- **`reproducibility/packages-r-pinned.R`**: Auto-generated list of all R packages from site-library with exact versions
- **`reproducibility/build.log`**: Validation report showing whether all specified packages are present in the built image
...
- **Never remove or edit** `reproducibility/packages-python-pinned.yaml` or `reproducibility/packages-r-pinned.R` manually - they are auto-generated
- **Never remove or edit** `reproducibility/build.log` - it is auto-generated by CI
...
- Preserve the multi-stage permission handling (root → NB_USER)
...
3. **Build validation fails**: Check reproducibility/build.log in the auto-generated PR for details on missing packages
